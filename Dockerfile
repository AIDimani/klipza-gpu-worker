...
RUN curl -L https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-linux64-lgpl.tar.xz \
    -o /tmp/ffmpeg.tar.xz \
    && tar -xJf /tmp/ffmpeg.tar.xz --strip-components=2 -C /usr/local/bin/ \
       ffmpeg-master-latest-linux64-lgpl/bin/ffmpeg \
       ffmpeg-master-latest-linux64-lgpl/bin/ffprobe \
